...
For a quick and easy local preview run `./utilites/lab-build` followed by `./utilites/lab-serve`. You can re-run `lab-build` and the new build will automatically be served by the existing serve container.

Check the `utilites/` folder for more helpful scripts.

=== Using Containers Locally (Recommended)
