...
* Modern software delivery is shaped by two opposing forces.
* On one side, security expectations have never been higher. Organisations are under pressure to prove where their software comes from, how it was built, and whether it can be trusted — across every stage of the supply chain.
* On the other side, developers face increasing cognitive load. Tool sprawl, fragmented workflows, and manual security steps slow delivery and create frustration when security is bolted on after the fact.
* Most organisations struggle to balance both. They invest heavily in security tooling or developer productivity, but rarely achieve both without friction.
* This is where platform engineering comes in.

=== Platform Engineering and Internal Developer Portals

* *Platform engineering* is the discipline of designing and operating a curated internal platform that abstracts complexity and delivers secure capablties to development teams.
* A platform engineering team builds and maintains Internal Developer Platforms (IDPs) that bring together the capablties and tools that developers need.
* These capabilities are exposed through a consistent, opinionated, self-service *Golden Paths Templates*.
* *Golden Paths Templates* are standardised, reusable, and secure workflows that guide developers through building, testing, and deploying software in the approved way. They reduce decision fatigue for developers while ensuring security and compliance are enforced by design.
* The outcome is twofold:
. developers focus on code, not infrastructure or security plumbing
. security and compliance are enforced by design, not as after-the-fact checks
...
* RHDH is based on the upstream link:https://www.backstage.io[Backstage,window=_blank] project created by Spotify.
...
image::add_new_user.png[add_new_user,align="center"]
...
image::create_user1.png[create_user1,align="center"]
...
image::join_group.png[join_group,align="center"]
...
image::join_groups.png[join_groups,align="center"]
...
image::select_set_password.png[select_set_password,align="center"]
...
image::set_password_screen.png[set_password_screen,align="center"]
...
* Rather than enabling features manually through the UI, you will configure Red Hat Developer Hub using GitOps and Infrastructure as Code practices, ensuring configuration is version-controlled, reviewable, and aligned with enterprise platform standards.
...
==== Plugin management in upstream Backstage
...
+
image::select_gitops_repo.png[select_gitops_repo,align="center"]
+
. Navigate to the `charts/Backstage` folder.
+
image::navigate_to_backstage.png[navigate_to_backstage,align="center"]
+
. Select *Edit* > *Edit single file*.
+
image::edit_single_file.png[edit_single_file,align="center"]
+
. Locate the commented block under the *dynamic.plugins* section and uncomment it using `CMD + /` (Mac) or `CTRL + /` (Windows/Linux).
+
image::uncomment_plugins.png[uncomment_plugins,align="center"]


==== Understanding the dynamic.plugins configuration
...
* Remove the existing guest provider block under auth.providers.
* Uncomment the oidc provider configuration and the signInPage setting using `CMD + /` (Mac) or `CTRL + /` (Windows/Linux) as shown below:

image::enable_auth.png[enable_auth,align="center"]

==== Understanding the authentication configuration
...
* In ur YAML, providers are configured under *catalog.providers* and are usually delivered by plugins.
...
image::uncomment_catalog.png[uncomment_catalog,align="center"]
...
=== Integration and Proxy Configuration
...
[source, role="execute"]
----
feat: Updare Red Hat Developer Hub Configuration 
----
* Click *Commit Changes* 

image::enable_integration_proxy.png[enable_integration_proxy,align="center"]
...
image::switch_to_backstage_app.png[switch_to_backstage_app,align="center"]

* *Argo CD* is configured to automatically synchronise every two minutes. To avoid waiting for the next sync cycle, click *Hard Refresh* to trigger an immediate reconciliation.

image::hard_refresh.png[hard_refresh,align="center"]
...
image::argo_cd_rollout.png[argo_cd_rollout,align="center"]
...
image::sign_in_with_oidc.png[sign_in_with_oidcalign="center"]
...
image::catalog_users.png[catalog_users,align="center"]
