...
* In this module you help customers answer a foundational supply-chain question: “Do we actually trust the software components inside our applications?”
* Modern applications are assembled rather than written. Container images typically bundle hundreds of upstream packages, transitive dependencies, and libraries, each carrying its own CVE and advisory exposure.
...
* That is the role of *RHTPA*, Red Hat’s supported distribution of the upstream trustify project in the *GUAC* community (Graph for Understanding Artifact Composition). 
...
* Authorization in *RHTPA* is determined entirely by client scopes present inside the OIDC access token.
...
=== Explore Red Hat Build of Keycloak (RHBK)
...
* The YAML defines two clients: *frontend* and *cli* 
...
* This identity is what Tekton Chains and other CI tools authenticate as when uploading SBOMs.
* *clientAuthenticatorType* is set to *client-secret* means this client can use the client credentials flow (machine-to-machine).
...
image::cli_client_scopes.png[cli_client_scopes,align="center"]

* Select *service_account_roles*, here you will find that the *cli* client is assigned the *tpa-manager* role as expected.

image::cli_account_roles.png[cli_account_roles,align="center"]
...
* *RHTPA* can be deployed using Helm or an Operator. In enterprise clusters, the Operator model is preferred because it manages CR lifecycle, upgrades and dependent components.
...
* Run the following command in the showroom terminal to create the secret:
...
. OID configuration to authenticate users and pipelines
...
* A Software Bill of Materials (SBOM) is a machine-readable inventory (CycloneDX, SPDX, etc.) describing all dependencies in a software artifact. It is the prerequisite for provenance, vulnerability correlation, and build attestation.
* An *attestation* is a signed, tamper-evident statement that proves something about how an artifact was produced.  
* Where an SBOM describes *what* is in the software, an attestation proves *how, when and by whom* it was built.  
* Attestations are generated during the pipeline and are later verified by policy engines to ensure the build process  
was trusted, repeatable and compliant.
* In this step, you simulate what a CI/CD pipeline does during a build: authenticate using the client_credentials OAuth 2.0 grant and then upload an SBOM to RHTPA.
...
images::rhtpa_packages.png[rhtpa_packages,align="center"]

* Switch to the *Vulnerabilities* tab. You will notice that the view is currently empty.

images::rhtpa_vulnerabilities.png[rhtpa_vulnerabilities,align="center"]

[NOTE]
====
Why is the vulnerability report empty? 
If the test-sbom entry shows zero vulnerabilities, or the dependency graph appears empty, this is expected in a fresh installation.
*Reason*: *RHTPA* relies on background importer jobs (configured in the YAML) to download large advisory datasets — including NVD, GitHub Advisories, and others.
...
images::rhtpa_importers.png[rhtpa_importers,align="center"]

* These import jobs can take quite a while to complete, so you may not see full results during today’s lab.
* Once imports are finished, you will see identified vulnerabilities, similar to the example below:

images::rhtpa_vulnerabilities_complete.png[rhtpa_vulnerabilities_complete,align="center"]

---

===  Module 1 Summary

* By completing this module, you configured identity in *RHBK*, deployed *RHTPA* and validated your first SBOM—establishing a reliable source of truth for software composition. This is a realistic starting point for consultants implementing early-stage supply-chain controls for customers.
