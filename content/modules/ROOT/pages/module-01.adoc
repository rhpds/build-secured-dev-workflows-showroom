...
* *Username*: {openshift_gitops_admin_user}
* *Password*: {openshift_gitops_server_url}
* After entering your credentials,click the *Sign In* button.
...
image::tpa_instance_ready.png[TPA Operator,align="center"]
...
* Run the following command to extract the external route for the RHTPA server:
...
* Click the *RHTPA Console* tab in the Showroom
...
[NOTE]
====
Why is the vulnerability report empty? If the test-sbom entry shows 0 vulnerabilities, or the dependency graph appears empty, this is expected behaviour for a fresh installation. 
