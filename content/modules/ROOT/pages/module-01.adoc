...
* In this module you help a customer answer a foundational supply-chain question: “Do we actually trust the software components inside our applications?”
...
* That is the role of *Red Hat Trusted Profile Analyzer (RHTPA)*, Red Hat’s supported distribution of the upstream trustify project in the *GUAC* community (Graph for Understanding Artifact Composition). 
...
* The YAML defines two clients: *frontend* named *cli* 
* The frontend client issues browser-based tokens for the Web UI. When a user logs in, any client scopes mapped to their identity can be embedded in that token.
...
* Client scopes again include the document permissions, so a service account based on cli can upload and manage SBOMs without a human logging in.
...
* In this lab, you will apply a Custom Resource (CR) that instructs the Operator to deploy all backend components. 

=== Configure the Client Secret

* Before the *RHTPA* application can start, it must be configured to trust the Identity Provider (IdP). 
* This requires creating a Kubernetes Secret that contains the OIDC Client ID and Client Secret used by the *cli* client.
...
. OIDC to authenticate users and pipelines
