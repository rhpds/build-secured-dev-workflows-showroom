...
* One critical challenge remains: “How do we prevent unverified or unsafe images from reaching staging and production?”
...
* Under the hood, they use the link:https://backstage.spotify.com/docs/portal/core-features-and-plugins/scaffolder[*Backstage scaffolder*,window=_blank] to automate the creation of repositories, configuration, and CI/CD resources based on platform-approved patterns, as you’ll see in the next section.

---

===  2.2 Log in back to RHDH

* If you are not already logged in to *RHDH*, click the *RHDH* Console tab in the Showroom and log in using:
...
== Step 3: Configure Git Signing in OpenShift Dev Spaces
...
=== 3.1 Create the tas-git-config ConfigMap
...
* Add the following atrributes under the development-tooling component to load the ConfigMap you created in the earlier step.
...
* Next, under commands add this entry to update the Git config for the current session to use the configured variables for commit  signing:
...
- id: init-git-config
    exec:
      label: "Configure Git for commit signing"
      component: development-tooling
      commandLine: |
        USERNAME=user1-devspaces
        ...
        git config --global gitsign.issuer "https://sso.{openshift_cluster_ingress_domain}/realms/sso"
        git config --global gitsign.rekor  "{tas_rekor_url}"
        git config --global gitsign.clientid "trusted-artifact-signer"
----

* Finally, update the devfile.yaml to use the variables injected from the ConfigMap and init the Git Config. by adding this post-start event that will command the command you just added in the devfile.
