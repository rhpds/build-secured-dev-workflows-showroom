...
* The *verify-commit* task runs a signature verification step equivalent to `git verify-commit`, ensuring the commit was signed by a trusted identity before the pipeline proceeds to the next stage.

image::Verify_Commit_Log.png[Verify Commit Task Log Output,link=self,window=blank]
