...
* Open RHDH in a Private/Incognito window using the link:{developer_hub_url}[*RHDH URL*^] 
...
=== 4.2 *`Optional Deep Dive:`* Development Environment as Code with Devfiles
...
* At the heart of this paradigm is the *Devfile*, a YAML configuration file used by **OpenShift Dev Spaces**.
...
+
image::my-quarkus-tkn_repo.png[gitlab_home_page,align="center",link=self,window=blank]
+
...
. Return to Red Hat Developer Hub (RHDH) in the Incognito Browse
. Click the link for *OpenShift Dev Spaces*. This will open your cloud-based development environment in a separate browser window.
+
...
+
. You will be asked to authorise access for the OpenShift Dev Spaces client. Accept the requested permissions by clicking *Allow selected permissions* to proceed.
+
...
image::94_Gitsign_Git_Global_Config.png[Git Global Configuration with Gitsign,link=self,window=blank]
...
----
+
image::97_Index_Markdown_Updated.png[Updated Index Markdown File,link=self,window=blank]
+
...
+
image::98_Terminal_Menu.png[Terminal Menu in Dev Spaces,link=self,window=blank]
+
...
+
image::90_Gitsign_Commit_Message.png[Git Commit Message with Gitsign,link=self,window=blank]
+
 The browser will prompt you for your credentials.  Enter `{user}` for the username and `{password}` for the password.

image::91_Gitsign_Credentials_Prompt.png[Gitsign OIDC Credentials Prompt,link=self,window=blank]

* You will then be redirected to another page that will present a verification code.  Copy this code into your clipboard.

image::92_Gitsign_Verification_Code.png[Gitsign Verification Code Screen,link=self,window=blank]
...
+
image::93_Gitsign_Commit_Signed.png[Successfully Signed Commit with Gitsign,link=self,window=blank]
+
...
image::112_secure_pipeline_in_progress.png[Secure Pipeline Execution in Progress,link=self,window=blank]

* In the meantime let's explore the different tasks in this pipeline.

image::113_Jenkins_Secure_Build_Pipeline.png[Secure Build Pipeline Overview,link=self,window=blank]
...
image::114_Jenkins_Click_On_Verify_Commit.png[Click on Verify Commit Task,link=self,window=blank]

* "Here in the log, you can see that we successfully validated the signature for the user who made the last code change.”
* "The *verify-commit* task executes the command link:https://git-scm.com/book/en/v2/Git-Tools-Signing-Your-Work[*git verify-commit*^] to verify that the signature is valid, before the pipeline moves to the next task." you point out.

image::115_Jenkins__Verify_Commit_Log.png[Verify Commit Task Log Output,link=self,window=blank]

== Task 2: Scan Source

* "After we package the code, running a static analysis to detect any potential bugs or code style violations is a good idea."
* The task called *scan* task,  utilizes a tool called link:https://www.sonarsource.com/products/sonarqube[*SonarQube*^] to analyze the source code and provide reports based on its quality.
...
* To access *SonarQube*, you use the following link:https://sonarqube-sonarqube.{OPENSHIFT_CLUSTER_INGRESS_DOMAIN}[*URL*^]  and log in with your credentials, **username:** `{adminuser}` and **password**: `{password}`.
...
image::54_SonarQube_Dashboard.png[SonarQube Analysis Dashboard,link=self,window=blank]
...
image::116_Jenkins_Build_Sign_Image.png[Build and Sign Image Task,link=self,window=blank]
...
image::59_Generated_Artifacts_Registery.png[Generated Artifacts in Quay Registry,link=self,window=blank]
...
image::119_Jenkins_Upload_SBOM_To_CycloneDX.png[Upload SBOM to CycloneDX Task,link=self,window=blank]
...
image::120_Jenkins_Upload_SBOM_To_TPA.png[Upload SBOM to TPA Task,link=self,window=blank]

* To access *Trusted Profile Analyzer (TPA)*, you use the following link:https://console-trusted-profile-analyzer.{OPENSHIFT_CLUSTER_INGRESS_DOMAIN}[*URL*^] and log in with your credentials, **username**: `{adminuser}` and **password**: `{password}`.
...
image::121_Search_TPA.png[Search TPA for SBOM,link=self,window=blank]
...
image::122_Results_TPA.png[TPA Search Results,link=self,window=blank]
...
image::123_Dependency_Analytics_TPA.png[TPA Dependency Analytics View,link=self,window=blank]
...
image::118_Jenkins_ACS_Image_Check_Task.png[ACS Image Check Task,link=self,window=blank]
...
image::117_Jenkins_ACS_Image_Scan_Task.png[ACS Image Scan Task Results,link=self,window=blank]
