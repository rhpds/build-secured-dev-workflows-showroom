...
* *RHTAS* is an enterprise deployment of the link:https://www.sigstore.dev[*Sigstore project*,window=_blank] project. 
...
==== What did the RHTAS Instance CR configure?

* The Securesign custom resource you just applied tells the RHTAS Operator how to stand up Fulcio, Rekor, Trillian and TUF, and how to trust your Keycloak realms. Here are the key parts worth calling out.
    ...
    config:
      OIDCIssuers:
        - ClientID: ${TAS_CHAINS_CLIENT_ID}
          Issuer: 'https://sso.apps.cluster-hswcg.dynamic.redhatworkshops.io/realms/${TAS_CHAINS_REALM}'
          IssuerURL: '${KEYCLOAK_URL}/realms/${TAS_CHAINS_REALM}'
          Type: email
        - ClientID: ${TAS_CHAINS_CLIENT_ID}
          Issuer: 'https://sso.apps.cluster-hswcg.dynamic.redhatworkshops.io/realms/${TAS_SSO_REALM}'
...
* It observes pipeline executions, captures build materials (provenance, SBOMs, image digests), and—when configured, it can automatically signs container images using Cosign.
...
* Run the following command to find the generated IMAGE_DIGEST
* Keep does value handy as we will use to investigate the log entries in Rekor in the next step

[source, role="execute"]
----
IMAGE_DIGEST=$(oc get taskrun -n tssc-app-ci \
  -o jsonpath='{.items[0].status.results[?(@.name=="IMAGE_DIGEST")].value}')
echo $IMAGE_DIGEST
----

* or you can switch to the OpenShift console, navigate to Pipelines -> Tasks and select the TaskRuns tab
* Scroll down to find the IMAGE_DIGEST value 

image::check_chains_task_run.png[Check Chains Task Run,align="center"]

---

=== Verify Certifiacte and Attestation in Rekor search UI

* Click the *Rekor Search UI* tab in the Showroom.
* After *Cosign* completes the signing operation, Rekor stores two complementary entries.
* The first is a hashedrekord, Rekor’s standard record type for a signature over a hash, which contains the *Cosign* signature over the image digest — proof that someone with a valid OIDC identity signed this image and that the content hasn’t been altered since.
* The second is an in-toto provenance statement, generated by Tekton, which captures how the image was built — which pipeline ran, when it finished, and the exact digest it produced.
* Choose *Log Index* from the *Attribute* drop down list
* Provide Log Index value as 1
* Click *Search*

image::rekor_log_index_1.png[rekor_log_index_1,align="center"]

* This first Rekor record represents the signature event.

image::rekor_log_index_1_details.png[rekor_log_index_1_details,align="center"]
...
[source]
----
sha256:796b8cea8048cc88777c... (truncated)
----

* This is the SHA-256 digest of the image Tekton built , it should match the IMAGE_DIGEST value you extracted in the previous step, prooving that the signature is tied to the task build output.
...
* At the bottom in the Verification section you can see:
+
[source,yaml]
----
inclusionProof:
...
* Expand the Attestation section to examine it's detais. This is a simplified version:
...
=== Verify the Signed image in Quay
